...
# 选择带宽(cv.ls)
bw1 <- npregbw(T_ij ~ W_ij, regtype = "ll", bwmethod = "cv.ls")
bw2 <- npregbw(T_ij ~ D_ij, regtype = "ll", bwmethod = "cv.ls")
bw3 <- npregbw(T_ij ~ W_ij_D_ij_ratio, regtype = "ll", bwmethod = "cv.ls")

# 设置输出文件名
output_file <- sprintf("R_results/Output_ls_Network_%s.txt", number)
...
# 保存图形为文件
ggsave(sprintf("R_results/Fig_ls_Network_%s_all.jpeg", number), p_combined, width = 7.08661, height = total_height, units = "in", dpi = 300)

# 保存图像，宽度为 180 mm (7.08661 inches)，分辨率为 300 dpi
ggsave(sprintf("R_results/Fig_ls_Network_%s_p1.jpeg", number), p1, width = 7.08661, height = 7.08661 / 1.5, units = "in", dpi = 300)
ggsave(sprintf("R_results/Fig_ls_Network_%s_p2.jpeg", number), p2, width = 7.08661, height = 7.08661 / 1.5, units = "in", dpi = 300)
ggsave(sprintf("R_results/Fig_ls_Network_%s_p3.jpeg", number), p3, width = 7.08661, height = 7.08661 / 1.5, units = "in", dpi = 300)